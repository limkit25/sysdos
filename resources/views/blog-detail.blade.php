...
<head>
    ...
    <title>{{ $blog->title }} - {{ $site_settings->site_name ?? 'Sysdos Digital' }}</title>
            ...
            <nav class="hidden xl:block">
                <ul class="flex items-center gap-8 text-sm font-bold uppercase tracking-widest">
                    ...
                    <li><a href="{{ route('home') }}#portfolio"
                            class="hover:text-primary transition-colors">Portfolio</a></li>
                    <li><a href="{{ route('home') }}#blog"
                            class="text-primary hover:text-primary transition-colors">Artikel</a></li>
                </ul>
            </nav>
    ...
    <section class="relative pt-32 pb-20 overflow-hidden">
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('storage/' . $blog->image) }}"
                class="w-full h-full object-cover blur-md opacity-20 dark:opacity-10" alt="Background">
            <div
        ...
        <div class="container relative z-10">
            <div class="flex flex-col items-center text-center max-w-4xl mx-auto" data-aos="fade-up">
                <a href="{{ route('home') }}#blog"
                    ...
                    <span class="text-xs font-bold uppercase tracking-widest">Kembali ke Artikel</span>
                </a>

                <div
                    class="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-full mb-6 font-bold text-xs uppercase tracking-widest">
                    <i class="fas fa-newspaper"></i> Artikel & Berita
                </div>

                <h1 class="text-4xl md:text-6xl font-extrabold text-gradient mb-8 leading-tight">
                    {{ $blog->title }}
                </h1>

                <div
                    class="flex items-center justify-center gap-6 text-sm text-lightgrey dark:text-slate-400 font-bold uppercase tracking-widest">
                ...
                <div class="w-full aspect-video rounded-[40px] overflow-hidden border-8 border-white/20 dark:border-white/5 shadow-2xl mt-16 group"
        ...
        <div class="container">
            <div class="grid lg:grid-cols-12 gap-16">
                ...
                <div class="lg:col-span-8" data-aos="fade-right">
                    <div
                        class="glass dark:glass-dark p-8 md:p-12 rounded-[40px] border border-white/10 shadow-xl overflow-hidden relative">
                        <div class="absolute top-0 left-0 w-full h-2 bg-linear-to-r from-primary via-accent to-primary">
                        </div>

                        <div
                            class="prose prose-lg dark:prose-invert max-w-none prose-headings:font-extrabold prose-headings:text-gradient prose-img:rounded-3xl prose-img:shadow-xl text-lightgrey dark:text-slate-300 leading-relaxed mb-12">
                            {!! $blog->content !!}
                        </div>

                        <!-- Share Section -->
                        <div
                            class="pt-10 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
                            <h5
                                class="font-extrabold text-darkblue dark:text-white m-0 uppercase tracking-widest text-sm">
                                Bagikan Artikel:</h5>
                            <div class="flex gap-4">
                                <a href="https://wa.me/?text={{ urlencode('Baca artikel menarik ini: ' . route('blog.show', $blog->slug)) }}"
                                    target="_blank"
                                    class="w-12 h-12 flex items-center justify-center bg-green-500 text-white rounded-2xl shadow-lg hover:scale-110 transition-transform">
                                    <i class="fab fa-whatsapp text-xl"></i>
                                </a>
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ route('blog.show', $blog->slug) }}"
                                    target="_blank"
                                    class="w-12 h-12 flex items-center justify-center bg-[#1877F2] text-white rounded-2xl shadow-lg hover:scale-110 transition-transform">
                                    <i class="fab fa-facebook-f text-xl"></i>
                                </a>
                                <a href="https://twitter.com/intent/tweet?text={{ urlencode($blog->title) }}&url={{ route('blog.show', $blog->slug) }}"
                                    target="_blank"
                                    class="w-12 h-12 flex items-center justify-center bg-black text-white rounded-2xl shadow-lg hover:scale-110 transition-transform">
                ...
                <div class="lg:col-span-4" data-aos="fade-left">
                    <div class="sticky top-32 space-y-8">
                        <!-- Solution CTA -->
                        <div
                            class="glass dark:glass-dark p-8 rounded-3xl border border-primary/10 shadow-xl text-center relative overflow-hidden group">
                            <div
                                class="absolute top-0 right-0 w-32 h-32 bg-primary/10 rounded-full blur-3xl -mr-16 -mt-16">
                            </div>
                            <div
                                class="w-20 h-20 bg-primary/10 rounded-3xl flex items-center justify-center text-primary text-4xl mx-auto mb-6 group-hover:scale-110 transition-transform">
                                <i class="fas fa-comments"></i>
                            </div>
                            <h5 class="font-extrabold text-darkblue dark:text-white mb-4">Butuh Solusi Digital?</h5>
                            <p class="text-lightgrey dark:text-slate-400 text-sm mb-8 leading-relaxed">Konsultasikan
                                kebutuhan sistem dan transformasi digital bisnis Anda bersama tim ahli kami sekarang.
                            </p>

                            @php $wa = preg_replace('/[^0-9]/', '', $site_settings->phone ?? ''); @endphp
                            @if($wa)
                                <a href="https://wa.me/{{ $wa }}?text=Halo,%20saya%20membaca%20artikel%20'{{ $blog->title }}'%20dan%20ingin%20konsultasi."
                                    target="_blank"
                                    class="w-full px-6 py-4 bg-linear-to-r from-primary to-accent text-white rounded-2xl font-bold shadow-xl shadow-primary/20 hover:scale-105 transition-all flex items-center justify-center gap-3">
                        ...
                        <!-- About Us -->
                        <div class="glass dark:glass-dark p-8 rounded-3xl border border-white/5 shadow-xl">
                            <h6
                                class="font-extrabold text-darkblue dark:text-white mb-6 uppercase tracking-widest text-xs flex items-center gap-2">
                                <span class="w-1 h-4 bg-primary rounded-full"></span> Tentang Kami
                            </h6>
                            <div class="flex items-center gap-4 mb-6">
                                ...
                                @else
                                    <div
                                        class="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center text-primary text-xl">
                                        <i class="fas fa-laptop-code"></i>
                                    </div>
                                @endif
                                <div>
                                    <p class="font-extrabold text-darkblue dark:text-white mb-0">
                                        {{ $site_settings->site_name ?? 'Sysdos Digital' }}</p>
                                    <p class="text-xs font-bold text-primary uppercase tracking-tighter m-0">Digital
                                        Solution Partner</p>
                                </div>
                            </div>
                            <p class="text-sm text-lightgrey dark:text-slate-400 leading-relaxed mb-0">
                                {{ $site_settings->address ?? 'Jakarta, Indonesia' }}
                            </p>
                        </div>

                        <!-- Newsletter CTA -->
                        <div
                            class="bg-linear-to-br from-darkblue to-darklight p-8 rounded-3xl shadow-2xl text-white relative overflow-hidden group">
                            <div
                                class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10">
                            </div>
                            <h5 class="font-extrabold mb-4 relative z-10">Ide Baru di Inbox Anda?</h5>
                            <p class="text-white/60 text-xs mb-6 relative z-10 leading-relaxed">Dapatkan wawasan
                                teknologi terbaru setiap minggunya.</p>
                            <a href="{{ route('home') }}#contact"
                                class="px-6 py-3 bg-primary text-white rounded-xl font-bold text-xs inline-block relative z-10 hover:bg-white hover:text-primary transition-all">Hubungi
                                Kami</a>
                        </div>
                    </div>
                </div>
